...
int main(void)
{
    ...
    for (int student = 0; student < STUDENTS; student++) {
        ...
        scanf("%d %d %d %d %d", &a[student][0], &a[student][1],
              &a[student][2], &a[student][3], &a[student][4]);
    }
    ...
    for (int i = 0; i < 5; i++) {
        printf("Student %d total and average: %d %lf\n",
               i + 1, total_score_s[i], (double)(total_score_s[i] / 5.0));
    }
    ...
    for (int i = 0; i < 5; i++) {
        printf("Quiz %d average, high and low score: %lf %d %d\n",
               i + 1, average_for_quiz[i], high_for_quiz[i], low_for_quiz[i]);
    }
